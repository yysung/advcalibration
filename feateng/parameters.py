...
def load_buzzer(flags, guesser_params, load=False):
    ...
    for ff in flags.features:
        ...
        if ff == "Wiki":
            ...
            feature.add_training("./data/wiki_text_2.json")
            buzzer.add_feature(feature)
            features_added.add(ff)
        ...
        if ff == "GuessInRun":
            from features import GuessInRunFeature
            feature = GuessInRunFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff)
        ...
        if ff == "AverageWordLength":
            from features import AverageWordLengthFeature
            feature = AverageWordLengthFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff)
            
        if ff == "Pronouns":
            from features import PronounsFeature
            feature = PronounsFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff)
            
        if ff == "Of":
            from features import HasOfFeature
            feature = HasOfFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff)
    

        if ff == "Plural":
            from features import PluralityFeature
            feature = PluralityFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff)

        if ff == "Space":
            from features import SpaceFeature
            feature = SpaceFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff)

        if ff == "Disam":
            from features import HasDisambiguationFeature
            feature = HasDisambiguationFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff)
        ...
        if ff == "Year":
            from features import YearFeature
            feature = YearFeature(ff)
            buzzer.add_feature(feature)
            features_added.add(ff)
